...
class CouponService(object):
    ...
    def cal_price(coupon, sal_amount):
        ...
        # 如果type是1，则返回优惠百分比乘以商品价格（取整 四舍五入）
            ...
            # 这里可以写成int(sal_amount * coupon.coupon_discount+0.5)达到想要四舍五入且取整的目的
            return round(sal_amount * coupon.coupon_discount)
    ...
    def user_coupon(coupon_id, token):
        ...
        if user_id is None:
            ...
            if res['user_id'] == user_id:
                db.execute("update user_coupon set status=%s,user_time=%s where coupon_id=%s",
                           (1, Job.get_time(), coupon_id))
                connection.commit()
            else:
                print("用户不匹配")
