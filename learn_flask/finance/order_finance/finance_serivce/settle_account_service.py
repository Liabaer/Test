...
class SettleAccountService(object):
    ...
    def add_settle_account(order_id, amount):
        ...
        db.execute(
            "insert into order_finance_settle_account(amount, status, order_id, create_time) values (%s,%s,%s,%s)",
    ...
    def settle_account(order_id, courier_id):
        ...
        db.execute("update order_finance_settle_account set status=%s,courier_id=%s where order_id=%s",
                   (1, courier_id, order_id))
        connection.commit()
