...
from learn_flask.courier.courier_flow.courier_service.vaild_check import ValidCheckUtils
        ...
        if not ValidCheckUtils.check_phone(courier.phone_number):
            ...
            if ValidCheckUtils.is_en(courier.name):
                db.execute(
                    ...
                    (courier.name, courier.status, courier.create_time, courier.phone_number, courier.password,
                     courier.is_ready,
                     courier.id_card))
                connection.commit()
            else:
    ...
    def courier_login(phone_number, password):
        ...
        if not ValidCheckUtils.check_phone(phone_number):
            ...
            if courier_res is None:
                ...
                RedisClient.create_redis_client().set("courier_login_id_" + str(courier_res['id']), str(token),
                                                      ex=86400)
