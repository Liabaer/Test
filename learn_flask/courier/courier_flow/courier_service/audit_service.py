...
from learn_flask.courier.courier_flow.courier_model.audit_log import AuditLog
from learn_flask.courier.courier_flow.courier_service.audit_log_service import AuditLogService
...
class AuditService(object):
    ...
    def reject_audit(audit_id, admin_id, reject_reason):
        ...
        if res is None:
            ...
            connection.commit()
            # 3. 调用审核日志类新增审核日志
            log = AuditLog(audit_id=audit_id, status=0, reject_reason=reject_reason, create_time=Job.get_time(),
                           admin_id=admin_id)
            AuditLogService.add_log(log)
