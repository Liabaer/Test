...
# 函数的参数分为形参（形式参数）和实参（实际参数） 形参又分为，位置参数、默认参数、可变参数、关键字参数
# 形参使在定义函数的时候给出的/定义函数的时候的变量叫形参（名字任意）
# 实参使在调用函数的时候给出的/真实的数据信息，掉用函数的时候传递叫实参
# 位置参数：形参和实参必须保持一致，最好按位置传参，如果位置不对应就指定说明
# 形参和实参可以不一致，如果不想使用默认的参数，在调用函数的时候可以指定（调用函数时，默认参数的值如果没有传入，则被认为是默 认值)
# a代表可变参数，a使元组数据类型
